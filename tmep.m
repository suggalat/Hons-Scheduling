...
X = [symbol_book(1);symbol_book(3)]; % Modulate data according to symbol_book
X = repmat(X,1,N);
...
for iter = 1:Nsym  
   [H1(:,:,iter),G,D(:,:,iter)] = SimRIS_v18_1(Environment,Scenario,Frequency,ArrayType,N,Nt,Nr,Tx_xyz,Rx1_xyz,RIS_xyz);
   G1(:,:,iter) = transpose(G);
   [H2(:,:,iter),G,D(:,:,iter)] = SimRIS_v18_1(Environment,Scenario,Frequency,ArrayType,N,Nt,Nr,Tx_xyz,Rx2_xyz,RIS_xyz);   
end
G1 = mean(G1,3);
H1 = mean(H1,3);
H2 = mean(H2,3);
...
noise1 = normrnd(0,sqrt(noise_pwr/2),size(x_s_ch1)) + normrnd(0,sqrt(noise_pwr/2),size(x_s_ch1))*1i;
noise2 = normrnd(0,sqrt(noise_pwr/2),size(x_s_ch2)) + normrnd(0,sqrt(noise_pwr/2),size(x_s_ch2))*1i;

x_s_noise1 = x_s_ch1 + noise1;
x_s_noise2 = x_s_ch2 + noise2;

snr_mean1 = 10*log10(abs(mean(x_s_ch1.^2)/mean(noise1.^2)));
% snr_mean1 = 10*log10(mean(abs(x_s_ch1.^2))/mean(abs(noise1.^2)));
% snr_mean2 = 10*log10(mean(abs(x_s_ch2.^2))/mean(abs(noise2.^2)));
snr_mean2 = 10*log10(abs(mean(x_s_ch2.^2)/mean(noise2.^2)));
...
SNR_Rx1 = abs(10*log10(10^13*(mean(K1).^2)));
SNR_Rx2 = abs(10*log10(10^13*(mean(K2).^2)));
%%  PLOTS
figure
